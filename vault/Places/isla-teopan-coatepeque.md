---
type: "place"
place_id: "isla-teopan-coatepeque"
name: "Isla Teopán, Lake Coatepeque"
aliases: []
place_kind: "Surveyed site"
atlas: true
latitude: 13.84417
longitude: -89.56306
coordinate_precision: "landmark"
coordinate_precision_label: "Site or landmark"
coordinate_precision_short_label: "Landmark"
coordinate_precision_description: "A known site, church, estate, community, or mapped feature."
coordinate_basis: "Figure 1 maps the Teopán site near the island's southwest shore; the article publishes no geodetic coordinate"
coordinate_note: "The marker represents Isla Teopán rather than the exact 1996 construction exposure."
location_visibility: "public"
periods:
  - "[[Periods/preclassic|Preclassic]]"
cultures: []
papers:
  - "[[Papers/teopan]]"
latest_study_year: 1996
latest_study_label: "Early 1996"
last_fieldwork_year: 1996
last_fieldwork_label: "Construction exposure and surface-sherd inspection reported in early 1996"
---

# Isla Teopán, Lake Coatepeque

## Overview

Construction near the shore of Isla Teopán exposed an archaeological deposit and a monumental potbelly sculpture in early 1996.

## Chronology

Surface ceramics and other reported finds support a Late Preclassic date for the site.

## How it was investigated

An inspection identified Chul- and Caynac-complex sherds on the surface after construction exposed the deposit. The article documents no controlled excavation and gives only limited context for the sculpture.

## What archaeologists found

The report lists the sculpture, four mushroom stones, Bolinas figurines, and ceramics. Amaroli identified the sculpture as a woman and proposed that potbelly sculptures represented pregnant women.

## Research history

### Early 1996 — Fieldwork

Construction exposed archaeological deposits, and Amaroli inspected the exposure and surface sherds.

**Evidence:** [[Papers/teopan#Page 1|teopan.pdf, PDF p. 1]]

## Location and uncertainty

The marker represents Isla Teopán rather than the exact 1996 construction exposure.

- **Coordinates:** 13.8442, -89.5631
- **Placed from:** Figure 1 maps the Teopán site near the island's southwest shore; the article publishes no geodetic coordinate
- **Precision:** landmark

## Sources

### teopan.pdf

**Citation:** PDF pp. 1–2 (printed pp. 51–52), Location and Discovery, figs. 1–3, and preliminary dating

[[Papers/teopan#Page 1|Open vault paper]] · [Open cited source](https://fundar.org.sv/referencias/teopan.pdf#page=1)

PDF p. 1 (printed p. 51), Location and Discovery and fig. 1, places the site near the shore of Isla Teopán, reports that construction exposed an archaeological deposit in early 1996, and says a potbelly sculpture remained at the site with at least one metre of soil over its battered head. It also reports surface sherds from the Late Preclassic Chul and Caynac complexes. PDF p. 2 (printed p. 52) says preliminary ceramic dating and other reported finds—four mushroom stones and several Bolinas figurines—accord with a Late Preclassic date; describes the sculpture; and identifies it as a woman. The article proposes a broader interpretation of potbelly sculptures as pregnant women, but documents no controlled excavation, site boundary, geodetic coordinate, or complete find context.
