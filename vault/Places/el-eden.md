---
type: "place"
place_id: "el-eden"
name: "El Edén"
aliases: []
place_kind: "Excavated site"
atlas: true
latitude: 13.725
longitude: -90.058
coordinate_precision: "approx"
coordinate_basis: "Cooperativa Nueva York; the report publishes no site boundary or coordinate"
coordinate_note: "Approximate area marker; it does not represent a published archaeological coordinate."
location_visibility: "public"
periods:
  - "[[Periods/preclassic|Preclassic]]"
cultures: []
papers:
  - "[[Papers/carmen]]"
latest_study_year: 1988
latest_study_label: "May–June 1988"
latest_study_kind: "fieldwork"
last_fieldwork_year: 1988
---

# El Edén

## Overview

The report identifies El Edén on land of Cooperativa Nueva York but gives no site boundary or archaeological coordinate. Its pottery was later than El Carmen’s Early Preclassic occupation.

## Chronology

Middle to Late Preclassic, based on pottery; fieldwork took place in May–June 1988.

## How it was investigated

Investigators surveyed, collected surface material, cleaned looter-trench profiles, and excavated one 2 × 1 metre test pit in arbitrary 25 cm levels.

## What archaeologists found

The test pit yielded eroded pottery, a prismatic obsidian blade, charcoal, obsidian, bone, and a metate fragment. The report identifies Middle Preclassic fugitive-polychrome sherds but notes that the second and third lots were disturbed.

## Research history

### May–June 1988 — Fieldwork

Investigators conducted surface collection, a small test excavation, and cleaning and documentation of a looter trench.

**Evidence:** [[Papers/carmen#Page 4|carmen.pdf, PDF pp. 4–5, 8]]

## Location and uncertainty

Approximate area marker; it does not represent a published archaeological coordinate.

- **Coordinates:** 13.7250, -90.0580
- **Placed from:** Cooperativa Nueva York; the report publishes no site boundary or coordinate
- **Precision:** approx

## Sources

### carmen.pdf

**Citation:** PDF pp. 4–5 (printed pp. 2–3), project scope and site comparison

[[Papers/carmen#Page 4|Open vault paper]] · [Open cited source](https://fundar.org.sv/referencias/carmen.pdf#page=4)

The report places El Edén on land of Cooperativa Nueva York and says the May–June 1988 project conducted small test excavations, survey, surface collection, and looter-trench cleaning there. It says the pottery from these investigations was later than El Carmen's and belonged mostly to the Middle and Late Preclassic. It does not publish an archaeological coordinate or site boundary.

### carmen.pdf

**Citation:** PDF p. 8 (printed p. 6), ‘El Edén’

[[Papers/carmen#Page 8|Open vault paper]] · [Open cited source](https://fundar.org.sv/referencias/carmen.pdf#page=8)

The El Edén section documents one 2 × 1 m test pit excavated in arbitrary 25 cm levels and cleaning between 2.00 and 2.25 m in a looter trench. It reports eroded pottery, a prismatic obsidian blade, charcoal, obsidian, bone, fugitive-polychrome Middle Preclassic sherds, and a metate fragment; the second and third test-pit lots were disturbed. The section provides no site dimensions or coordinate.
