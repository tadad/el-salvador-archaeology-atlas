---
type: "place"
place_id: "barra-ciega"
name: "Barra Ciega"
aliases: []
place_kind: "Mixed-period ceramic find locality"
atlas: true
latitude: 13.6205
longitude: -89.884
coordinate_precision: "approx"
coordinate_basis: "Broad placement east of Acajutla, following Haberland"
coordinate_note: "The archaeological source gives no reproducible coordinate or site boundary; the marker is an approximate coastal placement."
location_visibility: "public"
periods:
  - "[[Periods/preclassic|Preclassic]]"
  - "[[Periods/postclassic|Postclassic]]"
cultures: []
papers:
  - "[[Papers/casasola_panorama]]"
  - "[[Papers/haberland_marihua]]"
  - "[[Papers/haberland_sequences]]"
latest_study_year: null
latest_study_label: "Undated within the 1953–1954 and 1958 programs"
latest_study_kind: "fieldwork"
last_fieldwork_year: null
---

# Barra Ciega

## Overview

Barra Ciega is a ceramic find locality east of Acajutla. Its surface collection contained material assigned to more than one period.

## Chronology

Sources assign material to the Middle Preclassic Atiquizaya complex and the Postclassic Cihuatán phase, but no stratified chronology was established.

## How it was investigated

A mixed surface collection was studied; no excavation at Barra Ciega is documented. Agricultural disturbance prevented a secure reading of the deposits.

## What archaeologists found

The sources report pottery similar to the Atiquizaya complex and sherds assigned to the Cihuatán phase, but give no site-specific artifact inventory.

## Research history

### Undated within the 1953–1954 and 1958 programs — Fieldwork

Haberland documented a mixed surface collection but did not assign a site-specific collection year.

**Evidence:** [[Papers/haberland_sequences#Page 3|haberland_sequences.pdf, PDF pp. 3–4; haberland_marihua.pdf, PDF p. 18]]

## Location and uncertainty

The archaeological source gives no reproducible coordinate or site boundary; the marker is an approximate coastal placement.

- **Coordinates:** 13.6205, -89.8840
- **Placed from:** Broad placement east of Acajutla, following Haberland
- **Precision:** approx

## Sources

### haberland_sequences.pdf

**Citation:** PDF pp. 3–4 (printed pp. 23–24), Atiquizaya and Cihuatán complex discussions

[[Papers/haberland_sequences#Page 3|Open vault paper]] · [Open cited source](https://fundar.org.sv/referencias/haberland_sequences.pdf#page=3)

PDF p. 3 (printed p. 23) places Barra Ciega east of Acajutla, reports material similar to the Atiquizaya complex, and says agricultural disturbance had mixed the deposits so extensively that excavation would be needed for usable stratigraphic data. PDF p. 4 (printed p. 24) lists Barra Ciega among western localities with scattered Cihuatán-complex material. The article gives no reproducible coordinate, site boundary, site-specific field date, or artifact inventory.

### casasola_panorama.pdf

**Citation:** PDF p. 3 (printed p. 717), Middle Formative summary

[[Papers/casasola_panorama#Page 3|Open vault paper]] · [Open cited source](https://fundar.org.sv/referencias/casasola_panorama.pdf#page=3)

PDF p. 3 (printed p. 717) identifies Barra Ciega in Sonsonate as a Middle Formative site and associates the sites discussed there with the Providencia and Conchas phases. This secondary summary does not document Barra Ciega’s location, field method, or finds in detail.

### haberland_marihua.pdf

**Citation:** PDF p. 18 (printed p. 24), Barra Ciega surface-collection note

[[Papers/haberland_marihua#Page 18|Open vault paper]] · [Open cited source](https://fundar.org.sv/referencias/haberland_marihua.pdf#page=18)

PDF p. 18 (printed p. 24) reports Cihuatán-phase sherds at Barra Ciega near Acajutla and says the surface collection was too mixed to characterize securely. It does not identify individual vessel forms or a field date for the collection.
