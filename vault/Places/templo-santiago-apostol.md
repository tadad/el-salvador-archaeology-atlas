---
type: "place"
place_id: "templo-santiago-apostol"
name: "Templo Santiago Apóstol"
aliases: []
place_kind: "Potential archaeological locality · historic church investigation"
atlas: true
latitude: 13.9859
longitude: -89.6801
coordinate_precision: "landmark"
coordinate_precision_label: "Site or landmark"
coordinate_precision_short_label: "Landmark"
coordinate_precision_description: "A known site, church, estate, community, or mapped feature."
coordinate_basis: "Mapped Santiago Apóstol church landmark; the archaeological source gives no coordinate"
coordinate_note: "A national synthesis reports an investigation in 1998–1999 but gives no methods or results."
location_visibility: "public"
periods: []
cultures: []
papers:
  - "[[Papers/2012-escamilla-fowler-practica-arqueologia]]"
latest_study_year: 1999
latest_study_label: "1998–1999"
last_fieldwork_year: 1999
last_fieldwork_label: "1998–1999 investigation reported in a national synthesis"
---

# Templo Santiago Apóstol

## Overview

A historic church in Chalchuapa where the national Department of Archaeology conducted an investigation.

## Chronology

The investigation took place in 1998–1999. The cited summary does not date the church or any archaeological deposits.

## How it was investigated

The summary says that Claudia Ramírez directed the investigation's second phase, but it does not identify the field methods.

## What archaeologists found

The cited summary does not describe architectural features, deposits, or objects from the investigation.

## Research history

### 1998–1999 — Fieldwork

Two investigation phases documented church architecture and archaeological deposits.

**Evidence:** [[Papers/2012-escamilla-fowler-practica-arqueologia#Page 7|2012-escamilla-fowler-practica-arqueologia.pdf, PDF p. 7 (printed p. 433)]]

## Location and uncertainty

A national synthesis reports an investigation in 1998–1999 but gives no methods or results.

- **Coordinates:** 13.9859, -89.6801
- **Placed from:** Mapped Santiago Apóstol church landmark; the archaeological source gives no coordinate
- **Precision:** landmark

## Sources

### 2012-escamilla-fowler-practica-arqueologia.pdf

**Citation:** PDF p. 7 (printed p. 433), historical-archaeology overview

[[Papers/2012-escamilla-fowler-practica-arqueologia#Page 7|Open vault paper]] · [Open cited source](https://www.asociaciontikal.com/wp-content/uploads/2017/03/036_Escamilla_y_Fowler.pdf#page=7)

This page reports that the national Department of Archaeology investigated Templo Santiago Apóstol in Chalchuapa in 1998–1999 and that Claudia Ramírez directed the second phase. It does not describe the field method, finds, the church's date, or a coordinate.
