---
type: "place"
place_id: "el-rico-manzanilla"
name: "El Rico / Estero Manzanilla shell middens"
aliases: []
place_kind: "Surveyed site"
atlas: true
latitude: 13.442431
longitude: -87.825208
coordinate_precision: "landmark"
coordinate_precision_label: "Site or landmark"
coordinate_precision_short_label: "Landmark"
coordinate_precision_description: "A known site, church, estate, community, or mapped feature."
coordinate_basis: "Estero Manzanilla landmark; the source publishes no midden coordinate"
coordinate_note: "The source places one of two El Rico-area middens in Estero Manzanilla. The marker identifies the estuary, not either midden’s exact position or extent."
location_visibility: "public"
periods: []
cultures: []
papers:
  - "[[Papers/anales-53]]"
latest_study_year: 2000
latest_study_label: "July 2000"
last_fieldwork_year: 2000
last_fieldwork_label: "July 2000 archaeological reconnaissance"
---

# El Rico / Estero Manzanilla shell middens

## Overview

Two shell middens were identified in the El Rico area; the source places one in Estero Manzanilla.

## Chronology

The article discusses them among pre-Hispanic shell-midden precedents but gives no site-specific dating evidence.

## How it was investigated

Fabio Amador identified the two middens during an archaeological reconnaissance in July 2000. No excavation is documented.

## What archaeologists found

The source reports only the two shell middens. It does not describe their deposits, contents, size, or associated artifacts.

## Research history

### July 2000 — Fieldwork

Fabio Amador's Gulf reconnaissance identified the El Rico and Manzanilla shell middens.

**Evidence:** [[Papers/anales-53#Page 45|anales-53.pdf, PDF p. 45]]

## Location and uncertainty

The source places one of two El Rico-area middens in Estero Manzanilla. The marker identifies the estuary, not either midden’s exact position or extent.

- **Coordinates:** 13.4424, -87.8252
- **Placed from:** Estero Manzanilla landmark; the source publishes no midden coordinate
- **Precision:** landmark

## Sources

### anales-53.pdf

**Citation:** PDF p. 45 (printed p. 45), ‘Antecedentes de concheros prehispánicos en El Salvador’

[[Papers/anales-53#Page 45|Open vault paper]] · [Open cited source](https://www.cultura.gob.sv/download/revista-anales-53/?wpdmdl=3359#page=45)

The synthesis says Fabio Amador conducted an archaeological reconnaissance in the Bay of La Unión in July 2000 and identified two shell middens in the El Rico area, one in Estero Manzanilla. It gives no site-specific coordinate, dating evidence, description of finds or deposits, or excavation evidence.
